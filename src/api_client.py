...
class CoinGeckoClient:
    ...
    def get_coin_market_chart_range(
        ...
        """
        Obtém histórico de mercado em um intervalo de tempo.

        Args:
            coin_id (str): ID da criptomoeda
            from_timestamp (int): Timestamp UNIX de início
            to_timestamp (int): Timestamp UNIX de fim
            vs_currency (str): Moeda de referência

        Returns:
            Dict: Dados históricos (prices, market_caps, total_volumes)
        """
        ...
        try:
            # Respeitar rate limit (adicionando pequeno delay)
            time.sleep(1.5)  # CoinGecko free limit ~10-30 req/min

            response = self.session.get(endpoint, params=params, timeout=self.timeout)

            if response.status_code == 429:
                print(f"⚠️  Rate limit atingido para {coin_id}. Aguardando 60s...")
                time.sleep(60)
            ...
            if not data or "prices" not in data or not data["prices"]:
                # print(f"⚠️  Sem dados históricos para {coin_id}")
                return None

            return data

        except requests.exceptions.HTTPError as e:
            # 404 significa que a moeda pode não ter dados para esse range ou id mudou
            if e.response.status_code != 404:
                print(f"❌ Erro HTTP ao buscar histórico de {coin_id}: {e}")
            return None
        except Exception as e:
            print(f"❌ Erro ao buscar histórico de {coin_id}: {e}")
            return None
